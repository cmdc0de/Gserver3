...
int32_t calculate_native(wasm_exec_env_t exec_env, int32_t n, int32_t func1, int32_t func2) {
	...
	uint32_t argv[] = { static_cast<uint32_t>(n) };
	if (!wasm_runtime_call_indirect(exec_env, static_cast<uint32_t>(func1), 1, argv)) {
	...
	if (!wasm_runtime_call_indirect(exec_env, static_cast<uint32_t>(func2), 1, argv)) {
